...
class PayoutMethodsRepositoryImpl implements PayoutMethodsRepository {
  ...
  @override
  Future<Either<Failure, PayoutAccountEntity>> setDefaultPayoutMethod(String id) async {
    final result = await graphQLDatasource.mutate(
      Options$Mutation$markPayoutAccountAsDefault(
        variables: Variables$Mutation$markPayoutAccountAsDefault(
          id: id,
        ),
      ),
    );
    return result.map((r) => r.markPayoutAccountAsDefault.toEntity);
  }
