...
class AddBankTransferPayoutMethodFormCubit extends Cubit<AddBankTransferPayoutMethodFormState> {
  ...
  void submit({
    ...
    result.fold(
      (failure) => emit(state.copyWith(pageState: FormSubmissionState.error(failure.errorMessage))),
      (account) => emit(state.copyWith(pageState: FormSubmissionState.success(account))),
    );
  }
